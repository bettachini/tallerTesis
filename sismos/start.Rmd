...
sum(sismos_arg$Percibido == "FALSE") / nrow(sismos_arg)
```
...
### Algunas modificaciones sobre los datos
...
sismos_arg[, magnitud_lineal := 10^Magnitud]
```
...
summary(sismos_arg$magnitud_lineal)
```

```{r máximo de Magnitud}
max(sismos_arg$Magnitud)
```

```{r histograma de Magnitud con barras más finas}
hist(sismos_arg$Magnitud, main = "Histograma de Magnitud", xlab = "Magnitud", ylab = "Frecuencia", breaks = 50)
```
