...
max(sismos$Profundidad)
```

```{r proporción más profundos que 50 km}
# only those with Profundidad > 50
c( nrow(sismos[Profundidad > 50]), nrow(sismos[Profundidad < 50]) )
nrow(sismos[Profundidad > 50])/  nrow(sismos)
```

```{r histograma de Profundidad}
#hist(sismos$Profundidad, main = "", xlab = "Profundidad [km]", ylab = "Frecuencia", breaks = 50)
...
#  )) +
#    labs(x = "Magnitud [$M_w$]", y = "($Log_{10}$) sismos") +
#    geom_point() +
#    theme_classic(),
#  source = "select",
#  tooltip = c("key")
